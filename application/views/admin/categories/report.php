...
<?php $store_name = isset($store_name) ? $store_name : get_setting('store_name', 'ShopVista'); ?>
    ...
    <div class="report-toolbar">
        <div>
            <a href="<?= base_url('admin/categories'); ?>" style="background:#334155;color:white;text-decoration:none;padding:0.45rem 0.9rem;border-radius:6px;font-weight:700;margin-right:8px;display:inline-flex;align-items:center;">
                &larr; Kembali ke Kategori
            </a>
            <strong>Laporan Kategori Produk - <?= htmlspecialchars($store_name); ?></strong> • Total: <?= count($categories); ?> Kategori
        </div>
    ...
    <div class="report-container" id="reportArea">
        <div class="report-header">
            <div>
                ...
                <div style="font-size:0.8rem;color:#64748B;">Sistem Manajemen Toko Online & Kategori</div>
            </div>
        ...
        foreach ($categories as $c) {
            $total_produk_semua += intval($c->product_count);
        }
        ?>
        ...
        <div class="report-stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-val"><?= count($categories); ?> Kategori</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Total Produk Terhubung</div>
                <div class="stat-val" style="color:#059669;"><?= $total_produk_semua; ?> Produk</div>
            </div>
        </div>
        ...
        <table class="report-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight:700;color:#0F172A;"><?= htmlspecialchars($c->name); ?></td>
                    <td><code><?= htmlspecialchars($c->slug); ?></code></td>
                    <td><?= htmlspecialchars($c->icon ?: 'package'); ?></td>
                    <td class="text-center" style="font-weight:700;color:#4F46E5;">
                        <?= intval($c->product_count); ?> item
                    </td>
        ...
        <div class="report-signature">
            <div class="sign-box">
                ...
                <div style="font-size: 0.75rem; color: #64748B;">Staff Manajemen Konten</div>
            </div>
        </div>
    </div>
    ...
    function downloadReportExcel() {
        ...
        try {
            ...
            const wb = XLSX.utils.table_to_book(table, { sheet: "Laporan_Kategori" });
            XLSX.writeFile(wb, 'Laporan_Kategori_' + store + '_<?= date('Ymd_His'); ?>.xlsx');
        } catch (err) {
            console.error(err);
